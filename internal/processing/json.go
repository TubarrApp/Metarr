...
func processJSONFile(ctx context.Context, fd *models.FileData) error {
	...
	logger.Pl.D(2, "Beginning JSON file processing...")
	...
	if len(fd.MWebData.TryURLs) > 0 {
		if match := transformations.TryTransPresets(fd.MWebData.TryURLs, fd); match == "" {
			logger.Pl.D(1, "No presets found for video %q URLs %v", fd.OriginalVideoPath, fd.MWebData.TryURLs)
		}
	}
	...
	if edited, err := jsonRW.MakeJSONEdits(file, fd); err != nil {
		...
		logger.Pl.D(2, "Refreshing JSON metadata after edits were made...")
	...
	if len(fd.MetaOps.DateTags) > 0 || len(fd.MetaOps.DeleteDateTags) > 0 {
		...
		if err != nil {
			logger.Pl.E("Failed to make date tag edits for metadata in file %q: %v", file.Name(), err)
		} else if !ok {
			logger.Pl.D(1, "Did not make date tag edits for metadata, tag already exists?")
		}
	} else {
	...
	if fd.FilenameOps != nil && fd.FilenameOps.DateTag.DateFormat != enums.DateFmtSkip {
		if dateTag, err := dates.MakeDateTag(data, fd, fd.FilenameOps.DateTag.DateFormat); err != nil {
			...
			logger.Pl.I("Date tag %q already found in file name %q", dateTag, file.Name())
		} else {
...
func filetypeMetaCheckSwitch(ctx context.Context, fd *models.FileData) bool {
	...
	if outFlagSet {
		...
		logger.Pl.D(2, "Got output extension as %s", outExt)
	}
	...
	if outExt != "" && !strings.HasPrefix(outExt, ".") {
		...
		logger.Pl.D(2, "Added dot to outExt: %s, currentExt is %s", outExt, currentExt)
	}

	if outFlagSet && outExt != "" && !strings.EqualFold(outExt, currentExt) {
		logger.Pl.I("Input format %q differs from output format %q, will not run metadata checks", currentExt, outExt)
		return false
	}
